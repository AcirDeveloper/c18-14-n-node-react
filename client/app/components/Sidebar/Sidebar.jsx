        ...
        <aside className="h-full">
            <nav className={`relative h-full flex flex-col bg-[#112F63] rounded-tl-none rounded-bl-none rounded-tr-3xl rounded-br-3xl custom-shadow`}>   
                ...
                <div
                    ...
                    onClick={onExpanded}
                >   
                    {expanded ?<TiMediaPlayReverse className="text-sm text-white" />  : <TiMediaPlay className="text-sm text-white" />}
                ...
                <ul className={`flex-1 ${expanded?"px-[9px]":""} pt-[50px]`}>
                    {
                        sectionsSidebar.map(({ tag, text, icon, subsections }) => {
                            ...
                            const isSelectedTag = selectedTag === tag || (tag === "movements" && isMovementsSelected);
                            const itemSidebarClasses = `relative flex items-center py-2 
                                ...
                                ${!expanded && (tag === "movements" && isMovementsSelected) ? "custom-drop-shadow2" : ""}
                                ${isOpenSubsection ? "rounded-bl-none rounded-br-none" : ""}
                                ${isSelectedTag ? "bg-[#D8E6FE] text-[#112F63]" : "bg-[#112F63] text-[#ffffff]"}`;

                            return (
                                <div key={tag} className={`relative font-bold ${isOpenSubsection ?"custom-drop-shadow rounded-lg":""}`}>
                                    {/* itemSidebar */}
                                    <li className={itemSidebarClasses}
                                    ...
                                    {/* SubItems cuando sidebar esta expanded*/}
                                    {
                                        expanded && (
                                            <div className={`relative overflow-hidden transition-all duration-500 ease-in-out 
                                                ${isOpenSubsection ? "max-h-[1000px] opacity-100 rounded-lg" : "max-h-0 opacity-0"}`}
                                            >
                                                {subsections?.map(({ tag, text, icon },index) => (
                                                    <li key={tag}
                                                        ...
                                                        ${index < subsections.length - 1 ? "border-b border-black " : "rounded-lg"}`}
                                                        onClick={(event) => {
                                    ...
                                    {
                                        !expanded && (
                                            <div className={`absolute top-0 -right-[255px] overflow-hidden transition-all duration-500 ease-in-out bg-[#112F63] text-[#ffffff]
                                                ${isOpenSubsection ? "max-h-[1000px] opacity-100 rounded-lg rounded-tl-none rounded-bl-none" : "max-h-0 opacity-0"}`}
                                            >
                                                {subsections?.map(({ tag, text, icon },index) => (
                                                    <li key={tag}
                                                    ...
                                                    >
                                                        {icon}
                                                        <span className={`ml-3 text-sm w-[190px]`}>
                                                            {text}
                                                        </span>
                                                    </li>
                                                ))}
